...
import {
  ...
  X, RefreshCw, Loader2, AlertCircle,
...
export default function CommandesView({ onBack }: { onBack?: () => void }) {
  const { isAuthenticated } = useBoulanger();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`commandes-view-${boulangerieId}`)
      .on('postgres_changes',
  ...
  return (
    <div>
      {/* Sub-header sticky sous le header AppShell */}
      <div className="sticky top-14 z-10 bg-[#1A0F0A]/96 backdrop-blur border-b border-white/8 px-4 py-4 -mx-3 sm:-mx-4 md:-mx-6">
      ...
      <div className="pt-5 space-y-5">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-3 sm:-mx-4 md:-mx-6 px-3 sm:px-4 md:px-6">
          {([
