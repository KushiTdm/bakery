...
interface ProduitLocal {
  ...
  stockReel:       number;
  selected:        boolean;
  quantiteFlash:   number;   // ≤ stockReel TOUJOURS
  allergenes:      string[];
  savedInDb:       boolean;
}
...
function buildSuggestions(produits: ProduitLocal[], remise: number) {
  ...
  if (sel.length >= 2) {
    ...
    suggestions.push({ id: 'decouverte', label: 'Panier Découverte', emoji: '🌅', desc: '2 produits — idéal pour les curieux', produits: items, total: Math.round(total * 100) / 100, flash: Math.round(total * (1 - remise / 100) * 100) / 100 });
  }
  if (sel.length >= 3) {
    const items = sel.slice(0, Math.min(4, sel.length));
    const total = items.reduce((s, p) => s + p.prixVente, 0);
    suggestions.push({ id: 'gourmand', label: 'Panier Gourmand', emoji: '🧺', desc: `${items.length} produits — le best-seller`, produits: items, total: Math.round(total * 100) / 100, flash: Math.round(total * (1 - remise / 100) * 100) / 100 });
  }
  if (sel.length >= 4) {
    const total = sel.reduce((s, p) => s + p.prixVente, 0);
    suggestions.push({ id: 'grand', label: 'Grand Panier Anti-Gaspi', emoji: '🎁', desc: 'Tout l\'invendu — zéro gaspi', produits: sel, total: Math.round(total * 100) / 100, flash: Math.round(total * (1 - remise / 100) * 100) / 100 });
  }
  return suggestions;
}
...
export default function VueFlash() {
  ...
  // Resync quand todayStocks change sans perdre les sélections
  useEffect(() => {
  ...
  const saveConfig = useCallback(async () => {
    ...
    try {
      await fetch('/api/boulanger/profil', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ flash_remise_pct: remise, flash_heure_debut: heureDebut, flash_heure_fin: heureFin }),
      });
  ...
  const desactiverFlash = useCallback(async () => {
    ...
    setSyncState('saving');
    try {
      await fetch('/api/boulanger/flash', { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
  ...
  const hasStock    = produits.some(p => p.stockReel > 0);
  ...
  return (
    <div className="space-y-4 pb-24">
      ...
      <div className="pt-2 flex items-start justify-between">
        <div>
          <p className="text-white/40 text-[11px] uppercase tracking-widest font-medium">Flash invendus</p>
        ...
        {flashActif && selected.length > 0 && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className="relative rounded-2xl overflow-hidden border border-yellow-400/25 bg-yellow-400/8">
              <motion.div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent" animate={{ x: ['-100%', '100%'] }} transition={{ duration: 3, repeat: Infinity, ease: 'linear' }} />
              <div className="relative flex items-center gap-3 px-4 py-3">
                <motion.div animate={{ scale: [1, 1.12, 1] }} transition={{ duration: 1.8, repeat: Infinity }} className="bg-yellow-400 rounded-xl p-2 flex-shrink-0">
                  <Zap size={14} className="text-[#2C1810] fill-current" />
                </motion.div>
                <div className="flex-1">
                  <p className="text-yellow-300 font-semibold text-sm">Flash actif — {selected.length} produit{selected.length > 1 ? 's' : ''} en ligne</p>
                  <p className="text-yellow-300/60 text-xs">{dansFenetre ? `Dans la fenêtre horaire · ${heureDebut}h–${heureFin}h` : `Hors fenêtre horaire — visible dès ${heureDebut}h`}</p>
                </div>
                <button onClick={desactiverFlash} className="bg-red-500/20 border border-red-500/30 text-red-400 text-xs px-3 py-1.5 rounded-xl hover:bg-red-500/30 transition-colors flex-shrink-0">
                  Désactiver
                </button>
              </div>
            </div>
      ...
      {!dansFenetre && (
        <div className="flex items-start gap-3 bg-white/4 border border-white/8 rounded-2xl px-4 py-3">
          ...
          <p className="text-white/35 text-xs leading-relaxed">Flash programmé de <strong className="text-white/55">{heureDebut}h</strong> à <strong className="text-white/55">{heureFin}h</strong>. {selected.length > 0 ? ' Vos paniers sont prêts — ils s\'activeront automatiquement.' : ' Préparez vos paniers maintenant.'}</p>
        </div>
      )}
        ...
        {([
          ...
          return (
            <motion.button key={tab.id} whileTap={{ scale: 0.96 }} onClick={() => setActiveTab(tab.id)}
              className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-medium transition-all touch-manipulation ${isActive ? 'bg-[#C19A6B]/20 text-[#C19A6B] border border-[#C19A6B]/30' : 'text-white/40 hover:text-white/60'}`}>
        ...
        <div className="space-y-2.5">
          {!hasStock ? (
            <div className="flex flex-col items-center justify-center py-14 text-center">
              ...
              <p className="text-white/25 text-xs mt-1">Saisissez la production dans <span className="text-[#C19A6B]">Matin</span>, puis les restes dans <span className="text-[#C19A6B]">Stock</span>.</p>
            </div>
          ) : (
            <>
      ...
      {activeTab === 'paniers' && (
        <div className="space-y-3">
          {selected.length < 2 ? (
            <div className="flex flex-col items-center justify-center py-14 text-center">
              <span className="text-4xl block mb-4">🧺</span>
              <p className="text-white/50 font-medium text-sm">{produits.length === 0 ? 'Aucun produit disponible' : 'Sélectionnez au moins 2 produits'}</p>
              <p className="text-white/25 text-xs mt-1">Allez dans l'onglet <span className="text-[#C19A6B]">Stock</span> pour cocher des produits.</p>
            </div>
          ) : (
            <>
              <p className="text-white/35 text-xs px-1">Suggestions basées sur vos <strong className="text-white/55">{selected.length} produits</strong> sélectionnés.</p>
              {suggestions.map((sug, i) => (
                <motion.div key={sug.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="rounded-2xl border bg-white/4 border-white/8">
                  <div className="px-4 py-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="bg-green-500/10 border border-green-500/20 rounded-xl px-3 py-1.5">
                        <span className="text-green-400 text-xs font-medium">Client économise {(sug.total - sug.flash).toFixed(2)}€ (−{remise}%)</span>
                      </div>
              ...
              <div className="bg-white/3 border border-white/6 rounded-2xl overflow-hidden">
                <div className="px-4 py-3 border-b border-white/5 flex items-center gap-2">
                  ...
                  <p className="text-white/40 text-xs font-semibold uppercase tracking-wider">Ajustement manuel</p>
                </div>
                  ...
                  {selected.map(p => (
                    <div key={p.id} className="flex items-center gap-3 px-4 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-white/30 text-xs"><span className="text-[#C19A6B]/70 font-mono">{calcFlash(p.prixVente, remise).toFixed(2)}€</span> · max {p.stockReel}</p>
                      </div>
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        <motion.button whileTap={{ scale: 0.85 }} onPointerDown={() => adjustQuantite(p.id, -1)} disabled={p.quantiteFlash <= 0} className="w-8 h-8 rounded-lg bg-white/8 border border-white/10 flex items-center justify-center disabled:opacity-30">
                          <Minus size={12} className="text-white" />
                        </motion.button>
                        <span className="w-7 text-center text-sm font-bold font-mono text-[#C19A6B] tabular-nums">{p.quantiteFlash}</span>
                        <motion.button whileTap={{ scale: 0.85 }} onPointerDown={() => adjustQuantite(p.id, 1)} disabled={p.quantiteFlash >= p.stockReel} className="w-8 h-8 rounded-lg bg-[#C19A6B]/15 border border-[#C19A6B]/25 flex items-center justify-center disabled:opacity-30">
        ...
        <div className="space-y-4">
          <div className="bg-white/5 border border-white/8 rounded-2xl p-5 space-y-5">
            <div>
              ...
              <input type="range" min={10} max={70} step={5} value={remise} onChange={e => setRemise(parseInt(e.target.value))} className="w-full accent-yellow-400" />
              <div className="flex justify-between text-white/20 text-[10px] mt-1 px-0.5"><span>−10%</span><span>Recommandé : 30–50%</span><span>−70%</span></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="range" min={14} max={21} step={1} value={heureDebut} onChange={e => { const v = parseInt(e.target.value); if (v < heureFin) setHeureDebut(v); }} className="w-full accent-[#C19A6B]" />
              </div>
              <div>
                <div className="flex items-center justify-between mb-2">
                ...
                <input type="range" min={15} max={23} step={1} value={heureFin} onChange={e => { const v = parseInt(e.target.value); if (v > heureDebut) setHeureFin(v); }} className="w-full accent-[#C19A6B]" />
              </div>
            </div>
            <div className="bg-[#C19A6B]/8 border border-[#C19A6B]/20 rounded-xl px-4 py-3 flex items-center gap-3">
              ...
              <p className="text-[#C19A6B]/80 text-sm">Flash de <strong>{heureDebut}h</strong> à <strong>{heureFin}h</strong> · durée <strong>{heureFin - heureDebut}h</strong></p>
            </div>
            <div className="bg-white/4 border border-white/8 rounded-xl px-4 py-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-yellow-400 font-bold font-mono">{calcFlash(1.30, remise).toFixed(2)}€</span>
                <span className="bg-yellow-400/15 text-yellow-400 text-xs px-2 py-0.5 rounded-full border border-yellow-400/25">Client économise {(1.30 * remise / 100).toFixed(2)}€</span>
              </div>
            </div>
            <motion.button whileTap={{ scale: 0.97 }} onClick={saveConfig} disabled={savingConfig}
              ...
              {savingConfig ? <><Loader2 size={15} className="animate-spin" /> Enregistrement…</> : <><RefreshCw size={15} /> Enregistrer la configuration</>}
            </motion.button>
          </div>
          <div className="flex items-start gap-3 bg-white/3 border border-white/6 rounded-2xl px-4 py-3">
            ...
            <p className="text-white/30 text-xs leading-relaxed">Ces paramètres s'appliquent à toutes vos ventes flash. Les modifications sont visibles côté client en temps réel.</p>
          </div>
        </div>
      )}
    </div>
