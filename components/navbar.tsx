          ...
          <div className="flex items-center gap-2">
            {user ? (
              <div className="hidden md:flex items-center gap-2">
                <span className={`text-xs ${isTransparent ? 'text-white/60' : 'text-[#2C1810]/50'}`}>
                  {user.email}
                </span>
                <button onClick={logout} className={`p-2 transition-colors ${isTransparent ? 'text-white/70 hover:text-white' : 'text-[#2C1810]/50 hover:text-[#2C1810]'}`}>
                  <LogOut size={16} />
                </button>
              </div>
            ) : (
