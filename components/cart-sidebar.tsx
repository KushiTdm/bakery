    ...
    <AnimatePresence>
      {isCartOpen && (
        <>
          ...
          >
            {/* Header */}
            <div className="bg-[#2C1810] px-4 py-3 sm:py-5 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2 sm:gap-3">
                <ShoppingBag size={18} className="text-[#C19A6B]" />
                <h2 className="text-white font-bold text-sm sm:text-lg" style={{ fontFamily: 'Playfair Display, serif' }}>
                  {orderConfirmed
                ...
                {!orderConfirmed && totalItems > 0 && (
                  <span className="bg-[#C19A6B] text-white text-[10px] sm:text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
                    {totalItems}
                  </span>
                )}
              </div>
            ...
            {/* Corps — tout scrollable sur mobile pour que le checkout soit accessible */}
            <div className="flex-1 overflow-y-auto">
              {orderConfirmed ? (
                ...
                <>
                  ...
                  {/* Checkout — dans le scroll sur mobile */}
                  <div
                    className="border-t border-[#E8E0D5] bg-white px-3 sm:px-5 pt-3 sm:pt-4 space-y-2.5 sm:space-y-3"
                  ...
                  >
                    ...
                    <div className="space-y-1 text-xs sm:text-sm">
                      ...
                      <div className="flex justify-between text-[#2C1810] font-bold text-sm sm:text-base">
                        <span>Total TTC</span>
                    ...
                    {/* Date de retrait */}
                    <div>
                      <label className="text-[#2C1810]/60 text-[11px] sm:text-xs font-medium mb-1.5 flex items-center gap-1.5">
                    ...
                    {isPreOrder && (
                      <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 text-[11px] sm:text-xs text-amber-700 flex items-start gap-2">
                        <CalendarPlus size={12} className="text-amber-500 flex-shrink-0 mt-0.5" />
                        <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {plages.map(p => (
                          <button
                            ...
                            className={`px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-sm font-semibold transition-all border ${
                    ...
                    {/* Adresse */}
                    <div className="bg-[#F5F0E8] rounded-xl px-3 py-2 flex items-start gap-2">
                      <MapPin size={11} className="text-[#C19A6B] mt-0.5 flex-shrink-0" />
                      <div className="min-w-0">
                        <p className="text-[#2C1810]/50 text-[10px]">Retrait en boutique</p>
                        <p className="text-[#2C1810] text-[11px] sm:text-sm font-medium truncate">{adresseFormatted}</p>
                      </div>
                    </div>

                    {/* Statut connexion */}
                    {user ? (
                      <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs text-green-700 flex items-center gap-2">
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs text-amber-700">
                    ...
                    {submitError && (
                      <motion.div
                        ...
                        className="bg-red-50 border border-red-200 rounded-lg px-3 py-2 flex items-start gap-2"
                      >
                        <AlertCircle size={13} className="text-red-500 flex-shrink-0 mt-0.5" />
                        <p className="text-red-600 text-[11px] sm:text-xs">{submitError}</p>
                      </motion.div>
                    )}
                    ...
                    <motion.button
                      ...
                      className="w-full bg-[#2C1810] hover:bg-[#C19A6B] text-white py-3 sm:py-3.5 rounded-xl font-semibold text-xs sm:text-sm transition-colors duration-300 flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                      {isSubmitting
                    ...
                    <p className="text-center text-[#2C1810]/40 text-[10px] pb-1">
