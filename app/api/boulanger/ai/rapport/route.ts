...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Découplé du succès du rapport IA : même si le LLM a fail plus haut,
    // on tente quand même de générer des défis à partir de l'historique.
    try {
      const { generateAndPersistDefis } = await import('@/lib/defis-generate');
      const rapportInsight = rapportFinal
        ? (rapportFinal as {
            score?: number;
