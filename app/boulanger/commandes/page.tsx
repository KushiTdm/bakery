...
interface Order {
  id: string; commande_id: string; email: string; prenom: string | null;
  items: OrderItem[]; total: number; pickup_time: string | null;
  status: 'pending' | 'confirmed' | 'ready' | 'done'; created_at: string;
}
...
// Mapping front → DB (CHECK constraint de la table commandes)
const STATUS_TO_DB: Record<Order['status'], string> = {
  pending: 'en_attente', confirmed: 'confirmee', ready: 'prete', done: 'recuperee',
};
// Mapping DB → front
const DB_TO_STATUS = (s: string): Order['status'] => ({
  en_attente: 'pending', confirmee: 'confirmed', prete: 'ready',
  recuperee: 'done', retiree: 'done', annulee: 'done',
} as Record<string, Order['status']>)[s] ?? 'pending';
...
export default function CommandesPage() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/orders?boulangerie_id=${profile.id}&date=${today}`, {
        headers: { Authorization: `Bearer ${session.access_token}` },
        cache: 'no-store',
      });

      if (!res.ok) {
        const { error: e } = await res.json().catch(() => ({ error: 'Erreur inconnue' }));
        setError(e ?? 'Erreur lors du chargement');
        return;
      }

      const { commandes } = await res.json();
      setOrders((commandes ?? []).map((c: any) => ({
        id:          c.id,
    ...
    <div className="min-h-screen bg-[#1A0F0A] pb-20">
      <div className="sticky top-0 z-10 bg-[#1A0F0A]/95 backdrop-blur border-b border-white/8 px-4 py-4">
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          <div>
            ...
            <p className="text-white/30 text-xs">{lastRefresh ? `Mis à jour à ${formatTime(lastRefresh.toISOString())}` : 'Chargement…'}</p>
          </div>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 space-y-6">
        ...
        {loading && !orders.length && (
          <div className="flex flex-col gap-3">{[1,2,3].map(i => <div key={i} className="h-28 bg-white/4 rounded-2xl animate-pulse" />)}</div>
        )}
        ...
        <div className="flex flex-col gap-3">
          {orders.map(order => (
            <div key={order.id}
              className={`bg-white/4 border rounded-2xl overflow-hidden ${order.status === 'done' ? 'opacity-50' : ''}`}
